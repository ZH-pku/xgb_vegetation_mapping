...
%Computing Landsat Color Spaces Variables
...
%Color Enhancement Bands and Choices
BandsList={{'4','3','2'},{'5','4','3'},{'6','5','2'},{'5','6','2'},{'7','5','3'},{'7','5','4'},{'6','5','4'},{'5','7','1'}};
ColorSpaceList={'ycbcr','hsv'};
...
for bsi=1:length(BandsList)
    Bands=BandsList{bsi};
    disp('Reading Bands and Mid-step Down-sampling...');
    for bi=1:length(Bands)
        Imgtemp=calcSpecVars(Bands{bi},root,folderdirfromname,productname,filename_prefix,postfix,xlabels,ylabels,nrow,ncol,default); 
        scale=mid_resolution/ori_resolution;
        mid_nrow=round(nrow/scale);
        mid_ncol=round(ncol/scale);
        BandsImage(:,:,bi)=imresize(Imgtemp,[mid_nrow,mid_ncol],'nearest'); %direct subsampling using nearest neighbor method
    end
    clear Imgtemp;
    bandnames=[];
    for bi=1:length(Bands)
        bandnames=[bandnames,Bands{bi}];
    end

    for csi=1:length(ColorSpaceList)  
        ColorSpace=ColorSpaceList{csi};
        disp('Converting Color Space...');
        ImageCvt=cvtColorSpace(BandsImage,ColorSpace,default);
        [mid_nrow,mid_ncol,nstack]=size(ImageCvt);
        disp(' ');
        for i=1:nstack
            Image=ImageCvt(:,:,i);
            disp('----------------------------------------------------------------------------------------')
            disp(' ');
            disp(['Calculating Band: ',bandnames,', Color Space: ',ColorSpace,', Nstack: ',num2str(i)]);    
            disp(' ');
            t1=clock();
            ...
            folderdirto=[root,'/',resultfoldername,'/','Landsat_Color_Spaces'];
            if ~exist(folderdirto,'dir')
                mkdir(folderdirto);
            end
            postfix='.tif';

            filedirto=[folderdirto,'/',filename_prefix,bandnames,'_',ColorSpace,num2str(i),postfix];
            %[option, bbox] = make_option([bbox]);
            geotiffwrite(filedirto, bbox, TargetResult, 32);

            clear TargetResult
            disp(['Band: ',bandnames,', Color Space: ',ColorSpace,', Nstack: ',num2str(i),' finished!']);    
            disp(' ');    
            disp(' ');
        end
        disp(' ');
