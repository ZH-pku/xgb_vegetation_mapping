...
root='D:/Programs/VegetationMapping/paper writing/Submit Version/GitHub/feature_computing/landsat_features';                               %please change to your local computer workspace (the parent directory of 'codes' folder)
...
WindowDim1=9;                             %the moving window dimension for texture computing, an odd recommended
WindowDim2=81;                            %the moving window dimension for median and mean filters
...
for bi=1:length(Bands)
    ...
    disp('----------------------------------------------------------------------------------------')
    disp(' ');
    disp(['Vegetation Index: ',Band,'...']);    
    disp(' ');
    %Calculate Vegetation Index
    disp('Calculating Vegetation Index...')
    ...
    for method=1:13
        ...
        WindowDim=WindowDim2;           %Recommand an odd
        SplitDim=SplitDim2;
        ...
        parfor split_y=1:SplitNumY
            for split_x=1:SplitNumX
                ...
                if sum(sum(abs(SplitImage)))/single(SIDim*SIDim)==abs(default)
                    ...
                    SplitTextureStore{split_y,split_x}={};
                    continue;
                end
    ...
    %             MFBlock=medfilt2(MFBlock,[MFD2,MFD2]);
        ...
        folderdirto=[root,'/',resultfoldername,'/','Landsat_SpecVar_Textures'];
        if ~exist(folderdirto,'dir')
            mkdir(folderdirto);
        end
        postfix='.tif';
        filedirto=[folderdirto,'/','Texture_',Band,'_M',num2str(method),postfix];
        ...
        disp(['Method = ',num2str(method),' Finished!     Total Time Consuming = ',num2str(TotalTime),'s']);
        disp(' ');
        disp('****************************************************************************************');
        disp(' ');
    end
